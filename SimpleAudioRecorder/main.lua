...
local function recButtonPress ( event )
    ...
    elseif r then
        if r:isRecording() then
            ...
            -- Encode PCM recording to Opus format, then decode it back to simulate lossy compression
            local opusPath = filePath:gsub("%.[^%.]+$", ".opus")
            local encoded, encErr = opusModule.encodeFile( filePath, opusPath, r:getSampleRate() )
            if encoded then
                local decoded, decErr = opusModule.decodeFile( opusPath, filePath )
                if not decoded then
...
local rateUpButtonPress = function( event )
    local theRates = {8000, 12000, 16000, 24000, 48000}
...
local rateDownButtonPress = function( event )
    local theRates = {48000, 24000, 16000, 12000, 8000}
    if not r:isRecording () and not fSoundPlaying and not fSoundPaused then
--        r:stopTuner()
        local f = r:getSampleRate()
...
r:setSampleRate(16000) -- Initialize with a default rate supported by Opus
